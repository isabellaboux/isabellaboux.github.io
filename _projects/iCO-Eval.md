...
importance: 2
category: work
tags: [language, PCA, linear mixed modelling, inferential statistics, experimental design, ratings, matlab, python, visualization]
...
I showed human participants examples of direct and indirest replies to questions and ask tehm to provide various ratings about the reply. I analyze the  ratings and derived measures using **linear mixed-effects models (LMMs)**, **correlation analysis**, and **principal component analysis (PCA)**.
...
Because we know that ratings can be affected by very low level aspects of a sentence, I wanted my direct and inidrect replies to differ from each other only in terms of in/directness, but to be otherwise structurally identical to each other. This was achieved by varying the question preceeding the reply, while the reply itself remained undentical, including its pragmatic meaning (a statement in both cases).
...
| indirect     | **Are you bringing your cat to the vet?** | information query   | **It got wounded.**                  | statementn              | yes                          |
...
Because I was interested in making comparisons between teh SA-matehced and non-SA matched se, I also ensured and between responses that signified a *yes* or *no*, I ensured that length of the reply and number of content words was comparable.
...
I asked participants to view each question/answer pairs and asked them to rate them along several dimensions on a 7-point Likert scale:
...
The question/reply pairs wer presented in a randomized order and the direction of the Likert scale was inverted for half of the participants.
...
### Do the various ratings differ as a function of in/directess, SA-matching and polarity?

Because i have a hierarchical dataset with the same reply presented to different participants and teh same participant rating different sreplies, I modeled each rating dimension in **R** using **linear mixed-effects models** via the `lme4` package. 

```r
RATING ~ InDirectness * Polarity * SA_matching + (1 | Subject) + (1 | Item)
```

FOr each rating, I start modelling th e dtaa wih the simplest model and procressively add fixed factors. 
* Stepwise model building from **null model → full interaction model**
* Compared using **Likelihood Ratio Tests (LRT)**
I check model sanity by examining normality, homoscedasticity of model residuals.
...
The strongest pattern for all ratings i sthat idirect replies are perceived as less coheremt with their context, less predictable, less semantically similar to their context  and they are interpreted with less certainty. As expected, they are also perceived as less direct (sanity check). More fine grained modulations can be also detected in relation to complex combinations of indirectness, SA-matching and polarity.
...
Because the prior analysis showed strikign parallels between features, I asked whether these are correlated with one another. For each reply, I calsulated its average score for each feature and performed pairwise Pearson correlation between all features. Due to the number of correlations, Bonferroni correction is applied.
...
### Is indirectness a latent structure spannign all features?
...
Interestingly, a single component clearly explains 92% of varience in the features, with all features equally loading. Thsi indicates a **low-dimensional latent structure**, suggesting that indirectness reflects (or emerges from) a **joint cognitive signal**.
