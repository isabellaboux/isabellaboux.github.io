...
This project analyzed electroencephalography (EEG) recorded while participants produced the same kinds of one-word utterances in two different communicative contexts:
...
- in the **naming** condition, participants imagined taking a language test and named the selected object to an examiner;
- in the **request** condition, they imagined being a customer in a shop and requested the object from a salesperson.
...
The EEG data from the 64 channels is preprocessed according to standard event-related-potential practice. The aim is to go from a continuous signal recorded across 64 channels to averaged brain responses to the critical word (happy or blonde). For this purpose, the audio waveform were inspected to determine the exact onset of each spoken word. These voice-onset timestamps were aligned with the EEG recordings, providing a common temporal reference for analyzing neural activity immediately before speech production.
```mermaid
flowchart TD
